...
public class ReviewController extends CrudController<Review, ReviewDto, Long> {
    ...
    public ResponseEntity<List<ReviewDto>> filter(
        ...
        log.info("Received request to filter reviews for POS with ID '{}' and approved status '{}'.",
                posId, approved);
        List<Review> filteredReviews = reviewService.filter(posId, approved);
        List<ReviewDto> reviewDto = filteredReviews.stream().map(reviewDtoMapper::fromDomain).toList();
    ...
    public ResponseEntity<ReviewDto> approve(
        ...
        log.info("Received request to approve review with ID '{}' by user with ID '{}'.", id, userId);
        Review review = reviewService.getById(id);
        Review approvedReview = reviewService.approve(review, userId);
        ReviewDto reviewDto = reviewDtoMapper.fromDomain(approvedReview);
        return ResponseEntity.ok(reviewDto);
    }
}
